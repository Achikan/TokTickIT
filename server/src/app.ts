...
app.use(cors({ origin: true, credentials: true })); // Vite dev server + session cookie
app.use(express.json());
app.use(cookieParser());

// Lab 3 (Issue 18) — CSRF guard for mutating requests (api-spec.md §0).
// Runs before every route, including the auth routes.
...
// ---------------------------------------------------------------------------
// Issue 4 — Category list (now authenticated, any role: api-spec.md §2.1).
// GET /api/categories -> read categories from PostgreSQL via Prisma,
//   returning each { id, name } in predictable (id) order.
// ---------------------------------------------------------------------------
app.get("/api/categories", requireAuth, blockPendingPasswordChange, async (_req: Request, res: Response) => {
  try {
...
// ---------------------------------------------------------------------------
// Issue 8 — Related Systems reference data (authenticated, any role: api-spec §2.2).
// GET /api/related-systems -> only ACTIVE systems, in id order.
// ---------------------------------------------------------------------------
app.get("/api/related-systems", requireAuth, blockPendingPasswordChange, async (_req: Request, res: Response) => {
  try {
...
// The requester identity comes from the authenticated session (FR-07/BR-06);
// any client-supplied requesterId is ignored.
// ---------------------------------------------------------------------------
...
//   -> only Tickets owned by the authenticated Requester (BR-06); search/filter/sort/pagination per api-spec.md §3.
...
// only for the authenticated owner (FR-07/BR-06).
// A foreign or missing Ticket is rejected with 404 (non-disclosing, AC-03/BR-21).
// ---------------------------------------------------------------------------
app.get(
  "/api/tickets/:id",
  requireAuth,
...
// All endpoints enforce ownership from the authenticated session (BR-06) and
// reject foreign resources with a non-disclosing 404 (BR-21).
// ---------------------------------------------------------------------------
